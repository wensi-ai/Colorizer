...
def prepare_paired_input(task, id, input_names, processed_names, is_train=0):
    net_in = np.float32(imageio.imread(input_names[id]))/255.0
    if len(net_in.shape) == 2:
        net_in = np.tile(net_in[:,:,np.newaxis], [1,1,3])
    net_gt = np.float32(imageio.imread(processed_names[id]))/255.0
    ...
    for epoch in range(1,maxepoch):
        ...
        for id in range(num_of_sample): 
            ...
            if step % 100 == 0 :
                ...
                imageio.imsave("{}/training/step{:06d}_{:06d}.jpg".format(output_folder, step, id), 
        ...
        if epoch % save_freq == 0:
            for id in range(num_of_sample):
                ...
                if with_IRT:
                    ...
                    imageio.imsave("{}/{:04d}/predictions_{:05d}.jpg".format(output_folder, epoch, id),
                        np.uint8(np.concatenate([net_in[0,:,:,:3],prediction_main[0], prediction_minor[0],net_gt[0], confidence_map[0]], axis=1).clip(0,1) * 255.0))
                    imageio.imsave("{}/{:04d}/out_main_{:05d}.jpg".format(output_folder, epoch, id),np.uint8(prediction_main[0].clip(0,1) * 255.0))               
                    imageio.imsave("{}/{:04d}/out_minor_{:05d}.jpg".format(output_folder, epoch, id),np.uint8(prediction_minor[0].clip(0,1) * 255.0))   

                else:

                    imageio.imsave("{}/{:04d}/predictions_{:05d}.jpg".format(output_folder, epoch, id), 
                        np.uint8(np.concatenate([net_in[0,:,:,:3], prediction[0], net_gt[0]],axis=1).clip(0,1) * 255.0))
                    imageio.imsave("{}/{:04d}/out_main_{:05d}.jpg".format(output_folder, epoch, id), 
